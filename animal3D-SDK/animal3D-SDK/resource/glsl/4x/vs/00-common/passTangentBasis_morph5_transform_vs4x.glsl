...
sMorphTarget lerp(in sMorphTarget m0, in sMorphTarget m1, in float u)
{
	...
	m.tangent = nlerp(m0.normal, m1.normal, u);
...
//	gl_Position = aPosition;
	...
	int i0 = int(t) % 5;
	int i1 = (i0 + 1) % 5; 
	int iN = (i1 + 1) % 5; //iN for iNext
	int iP = (i0 + 4) % 5; //iP for iPrevious //also would technically be (i0 - 1 + 5)

	sMorphTarget morphTarget;
	morphTarget = aMorphTarget[0];
	...
	vec3 tmp = cross(k.normal.xyz, k.tangent.xyz);
